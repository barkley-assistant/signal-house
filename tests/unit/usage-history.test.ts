...
describe("queryUsageAggregate", () => {
  test("returns null when the window has no rows at all", () => {
    const db = openDb();
    expect(queryUsageAggregate(db, utcDaysAgo(30), utcDay(), { rates: new Map(), enabled: false })).toBeNull();
    db.close();
  });
  ...
  test("sums day-level totals per source across the window", () => {
    ...
    const agg = queryUsageAggregate(db, utcDaysAgo(7), utcDay(), { rates: new Map(), enabled: false })!;
  ...
  test("slices to the window (older days excluded)", () => {
    ...
    const week = queryUsageAggregate(db, utcDaysAgo(7), utcDay(), { rates: new Map(), enabled: false })!;
    expect(week.totalSessions).toBe(1);
    expect(week.totalCost).toBeCloseTo(1, 5);
    expect(week.byModel.map((m) => m.model)).toEqual(["Weekmodel"]);

    const ninety = queryUsageAggregate(db, utcDaysAgo(90), utcDay(), { rates: new Map(), enabled: false })!;
